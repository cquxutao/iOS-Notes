...
typedef NS_ENUM(NSInteger, UIDeviceBatteryState) {
    ...
    UIDeviceBatteryStateUnplugged,   // on battery, discharging //没充电
    UIDeviceBatteryStateCharging,    // plugged in, less than 100%
    UIDeviceBatteryStateFull,        // plugged in, at 100%
} __TVOS_PROHIBITED;              // available in iPhone 3.0

typedef NS_ENUM(NSInteger, UIUserInterfaceIdiom) { //什么玩意
...
#endif
//http://www.jianshu.com/p/973eff69d2de
@property(nonatomic,readonly,strong) NSString    *name;              // e.g. "My iPhone"
@property(nonatomic,readonly,strong) NSString    *model;             // e.g. @"iPhone", @"iPod touch"//设备类型
@property(nonatomic,readonly,strong) NSString    *localizedModel;    // localized version of model//本地设备模式
@property(nonatomic,readonly,strong) NSString    *systemName;        // e.g. @"iOS"
@property(nonatomic,readonly,strong) NSString    *systemVersion;     // e.g. @"4.0"

//http://www.jianshu.com/p/973eff69d2de
//默认这个好像是产生了的，所以beginGeneratingDeviceOrientationNotifications、endGeneratingDeviceOrientationNotifications好想没什么用
...
- (void)beginGeneratingDeviceOrientationNotifications __TVOS_PROHIBITED;      // nestable//开始生成 设备旋转 通知
- (void)endGeneratingDeviceOrientationNotifications __TVOS_PROHIBITED;//结束 设备旋转通知
...
UIKIT_EXTERN NSNotificationName const UIDeviceOrientationDidChangeNotification __TVOS_PROHIBITED; //为什么不注册这个通知，也能拿到orientation
UIKIT_EXTERN NSNotificationName const UIDeviceBatteryStateDidChangeNotification   NS_AVAILABLE_IOS(3_0) __TVOS_PROHIBITED;
UIKIT_EXTERN NSNotificationName const UIDeviceBatteryLevelDidChangeNotification   NS_AVAILABLE_IOS(3_0) __TVOS_PROHIBITED;//电池电量值
UIKIT_EXTERN NSNotificationName const UIDeviceProximityStateDidChangeNotification NS_AVAILABLE_IOS(3_0);//近距离传感器(比如设备贴近了使用者的脸部)
