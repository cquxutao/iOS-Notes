...
#if UIKIT_DEFINE_AS_PROPERTIES//下一个响应者，在实现中，一般会返回父级对象
@property(nonatomic, readonly, nullable) UIResponder *nextResponder;
#else
- (nullable UIResponder*)nextResponder;
#endif

#if UIKIT_DEFINE_AS_PROPERTIES//获取一个布尔值，指定对象是否可以获取焦点
@property(nonatomic, readonly) BOOL canBecomeFirstResponder;    // default is NO
#else
- (BOOL)canBecomeFirstResponder;    // default is NO
#endif
- (BOOL)becomeFirstResponder;//把对象设置为 firstResponder 对象

#if UIKIT_DEFINE_AS_PROPERTIES//对象是否可以取消 firstResponder 对象
@property(nonatomic, readonly) BOOL canResignFirstResponder;    // default is YES
#else
- (BOOL)canResignFirstResponder;    // default is YES
#endif
- (BOOL)resignFirstResponder;//取消对象为 firstResponder 对象,常用在键盘中

#if UIKIT_DEFINE_AS_PROPERTIES//指示对象是否为第一响应者，这里的第一响应者就是当前有焦点的对象，叫法挺奇怪的，第一次看到真还难以理解这个叫法所表达的意思
@property(nonatomic, readonly) BOOL isFirstResponder;
#else
- (BOOL)isFirstResponder;//当自己是第一响应者的时候，可以执行这个函数，做些简单的事情
#endif
